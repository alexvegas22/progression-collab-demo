	...
	<nav class="navbar justify-content-between navbar-dark bg-dark">
		...
		<div id="navDroite">
			<div v-show="!page_login">
				<button v-if="token" type="button" class="btn btn-outline-secondary navBtn" @click="déconnexion">{{ $t('menu.déconnexion') }}</button>
				<button v-else type="button" class="btn btn-outline-secondary navBtn" @click="connexion">{{ $t('menu.connexion') }}</button>
			</div>
			<label>

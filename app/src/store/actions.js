...
async function decodeTokenRessource( {commit, state }) {
	try {
		const token_décodé = jwt_decode(state.tokenRessource);
		if (token_décodé.typeRessource) {
			commit("setUserRessource", token_décodé.username);
			commit("setUriRessource", token_décodé.uri);
		}
	} catch (e) {
		commit("setUserRessource", null);
		commit("setUriRessource", null);
		return;
	}
}
